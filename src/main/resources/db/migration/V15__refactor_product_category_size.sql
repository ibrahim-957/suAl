...
SELECT DISTINCT size, TRUE, NOW(), NOW()
FROM products
...
WHERE ps.label = p.size;
ALTER TABLE products
    ALTER COLUMN size_id SET NOT NULL;
ALTER TABLE products
    ADD CONSTRAINT fk_product_size FOREIGN KEY (size_id) REFERENCES product_sizes (id);
ALTER TABLE products
    DROP COLUMN size;
...
UPDATE products p
SET sell_price = (SELECT pr.sell_price
                  FROM prices pr
